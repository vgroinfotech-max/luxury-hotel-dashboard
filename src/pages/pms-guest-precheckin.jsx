...
export default function GuestPreCheckin() {
  ...
  async function runScan() {
    ...
    setScanning(false);
    setOcrDone(true);
    setOcrData(OCR_MOCK[docType] || OCR_MOCK.Aadhaar);
  }
  ...
  async function handleCTA() {
    ...
    if (step === 1) { if (!ocrDone) { runScan(); } else { setStep(2); } return; }
    if (step === 2) { if (!visaDone) { runVisaCheck(); } else { setStep(3); } return; }
    if (step === 3) { setStep(4); return; }
    if (step === 4) { setSubmit(true); await sleep(1400); /* navigate to arrival page */ }
  }
              ...
              {!ocrDone && !scanning && (
                <div>
                  <div onClick={()=>fileRef.current.click()}
                    ...
                    <input ref={fileRef} type="file" accept="image/*,.pdf" style={{display:"none"}} onChange={runScan}/>
